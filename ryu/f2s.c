...
RYU_INLINE uint64_t float_pow5_inv_split(size_t index){
  assert(index < 31);
  static const uint64_t table[31] = {
...
RYU_INLINE uint64_t float_pow5_split(size_t index){
  assert(index < 47);
  static const uint64_t table[47] = {
...
RYU_INLINE uint32_t pow5Factor(uint32_t value) {
  uint32_t count = 0;
  for (;;) {
    assert(value != 0);
...
RYU_INLINE bool multipleOfPowerOf5(const uint32_t value, const uint32_t p) {
  return pow5Factor(value) >= p;
}

// Returns true if value is divisible by 2^p.
RYU_INLINE bool multipleOfPowerOf2(const uint32_t value, const uint32_t p) {
...
RYU_INLINE uint32_t mulShift(const uint32_t m, const uint64_t factor, const int32_t shift) {
  assert(shift > 32);
...
RYU_INLINE uint32_t mulPow5InvDivPow2(const uint32_t m, const uint32_t q, const int32_t j) {
  return mulShift(m, float_pow5_inv_split(q), j);
}

RYU_INLINE uint32_t mulPow5divPow2(const uint32_t m, const uint32_t i, const int32_t j) {
...
RYU_INLINE floating_decimal_32 f2d(const uint32_t ieeeMantissa, const uint32_t ieeeExponent) {
  int32_t e2;
  uint32_t m2;
  if (ieeeExponent == 0) {
...
RYU_INLINE int to_chars(const floating_decimal_32 v, const bool sign, char* const result) {
