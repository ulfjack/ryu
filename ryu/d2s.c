...
#if RYU_DIVCONST
// Manually replace division-by-constant by a multiplication and shifts.
...
//#else
  ...
  const uint32_t b00Hi = (uint32_t)(((uint64_t)aLo * bLo) >> 32);
  ...
  return b11 + mid1Hi + mid2Hi;
//#endif
}

// NB:
// The multipliers and shift values here are the ones generated by clang x64
...
static inline uint64_t div10(const uint64_t x) { // optimize this!
...
#else
    uint64_t qp = div100(vp);
    uint64_t qm = div100(vm);
    if (qp > qm) {
      const uint64_t qr = div100(vr);
      roundUp = (vr - 100 * qr) >= 50;
      vr = qr;
      vp = qp;
      vm = qm;
      removed += 2;
